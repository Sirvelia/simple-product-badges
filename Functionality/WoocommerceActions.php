...
class WoocommerceActions
{
	...
	public function __construct($plugin_name, $plugin_version)
	{
		...
		add_filter('woocommerce_sale_flash', '__return_false', 3);
	}
	...
	public function show_custom_bagde()
	{
		...
		//Si en un producto esta desactivado
		...
		$badge_id = $single_only ? $product->get_id() : $this->get_badge_id();
	...
	private function badge_type_new($badge_id, $product)
	{
		...
		if ((time() - (60 * 60 * 24 * $new_arrival_interval)) < $created) {
			...
			echo '<span class="onsale pb-absolute pb-py-1 pb-px-2 pb-m-2 pb-' . esc_html($position) . '-0 pb-top-0" 
			style="font-size: ' . esc_html($text_size) . 'px; color: ' . esc_html($text_color) . '; background-color: ' . esc_html($bg_color) . ' ">' . esc_html($name) . '</span>';
		} else if ($product->is_on_sale()) {
			echo '<span class="onsale pb-absolute pb-py-1 pb-px-2 pb-m-2 pb-right-0 pb-top-0">Sale!</span>';
		}
	}

	private function badge_type_lowinstock($badge_id, $product)
	{
		...
		if ($low_stock && $have_variations) {
			echo '<span class="onsale pb-group pb-cursor-pointer pb-absolute pb-py-1 pb-px-2 pb-m-2 pb-inline-block pb-' . esc_html($position) . '-0 pb-top-0" style="font-size: ' . esc_html($text_size) . 'px; color: ' . esc_html($text_color) . '; background-color: ' . esc_html($bg_color) . ' ">' . esc_html($name) . '
	...
	private function badge_type_custom($badge_id, $product)
	{
		...
		$current_date = strtotime(wp_date("Y-m-d", time()));

		if (!carbon_get_post_meta($badge_id, 'badge_custom_set_date')  || ($current_date >= $initial_date && $current_date <= $end_date)) {
			$name = carbon_get_post_meta($badge_id, 'badge_text');
			$text_size = carbon_get_post_meta($badge_id, 'badge_text_size') ? carbon_get_post_meta($badge_id, 'badge_text_size') : '12';
			$text_color = carbon_get_post_meta($badge_id, 'badge_text_color') ? carbon_get_post_meta($badge_id, 'badge_text_color') : '#000000';
			$bg_color = carbon_get_post_meta($badge_id, 'badge_bg_color') ? carbon_get_post_meta($badge_id, 'badge_bg_color') : 'transparent';
			...
			echo '<span class="onsale pb-absolute pb-py-1 pb-px-2 pb-m-2 pb-' . esc_html($position) . '-0 pb-top-0" 
				style="font-size: ' . esc_html($text_size) . 'px; color: ' . esc_html($text_color) . '; background-color: ' . esc_html($bg_color) . ' ">'
				. esc_html($name) . '</span>';
		} else if ($product->is_on_sale()) {
			echo '<span class="onsale pb-absolute pb-py-1 pb-px-2 pb-m-2 pb-right-0 pb-top-0">Sale!</span>';
		}
	...
	private function badge_type_sale($badge_id, $product)
	{
		$name = carbon_get_post_meta($badge_id, 'badge_text');
		$text_size = carbon_get_post_meta($badge_id, 'badge_text_size') ? carbon_get_post_meta($badge_id, 'badge_text_size') : '12';
		$text_color = carbon_get_post_meta($badge_id, 'badge_text_color') ? carbon_get_post_meta($badge_id, 'badge_text_color') : '#000000';
